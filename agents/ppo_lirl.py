...
def optimize_rew(self):
        ...
        for e in range(self.epoch):
            ...
            for sample in generator:
                ...
                # Clipped Surrogate Objective
                log_prob_act_batch = dist_batch.log_prob(act_batch)
                log_prob_act_batch.requires_grad = False
                value_batch.requires_grad = False
                log_prob_act_batch *= self.inv_temp
                

                #TODO: need to figure this out:
                next_val = value_batch[1:]
                rew_hat = self.rew_model(obs_batch)


                adv_dones = rew_hat[flt] - value_batch[flt] + np.log(n_actions)
                adv = rew_hat[~flt] - value_batch[~flt] + self.gamma * next_val[~flt]
