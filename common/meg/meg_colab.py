...
def state_occupancy(pi: np.ndarray, T: np.ndarray, gamma: float, mu: np.ndarray, max_iterations = 1000, d = None):
    ...
    for iter in range(max_iterations):
      old_d = d.copy()
      d = mu + gamma * einops.einsum(pi, T, d, 'prev_states actions, prev_states actions states, prev_states -> states')
...
def state_action_occupancy(pi: np.ndarray, T: np.ndarray, gamma: float, mu: np.ndarray = None, max_iterations = 1000, d = None):
    """
...
def policy_evaluation(pi: np.ndarray, U: np.ndarray,  T: np.ndarray, gamma: float, mu: np.ndarray = None, max_iterations = 1000, d = None):
    """
...
def value_iteration_step(U: np.ndarray, T: np.ndarray, gamma, V = None):
  ...
  pi[np.arange(num_states), np.argmax(gamma*einops.einsum(T, V, 'states actions next_states, next_states -> states actions'), axis = -1)] = 1
  return V, pi
...
def soft_value_iteration(U: np.ndarray, T: np.ndarray, beta=1.0, gamma: float = 0.9, Q = None, max_iterations = 10000,):
  ...
  for iter in range(max_iterations):
    ...
    Q = np.expand_dims(U, -1) + gamma*einops.einsum(T, (1/beta)*logsumexp(beta*Q, axis=-1), 'states actions next_states, next_states -> states actions')
...
def unknown_utility_meg(pi: np.ndarray, T: np.ndarray, gamma: float = 0.9, mu: np.ndarray = None, max_iterations = 10000, lr = 0.1):
    """
    ...
    assert T.shape == (num_states, num_actions, num_states), "Transition probability table must have shape (num_states, num_actions, num_states)"
    if mu is None:
